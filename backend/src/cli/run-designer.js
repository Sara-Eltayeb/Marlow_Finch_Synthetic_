...
const run = { run_id: runId, phase: "designer", selected_user_id: userId, status: "started" };
...
try {
  ...
  const researchBrief = await runResearcher({
    user: selected.user,
    currency,
  ...
  console.log(JSON.stringify(researchBrief, null, 2));
  const strategy = await runDesigner({
    researchBrief,
    geminiApiKey: process.env.GEMINI_API_KEY,
    model,
  });
  ...
  run.completed_at = new Date().toISOString();
  console.error(`Designer phase failed: ${error.message}`);
  process.exitCode = 1;
}

await mkdir("evidence-runs", { recursive: true });
await writeFile(`evidence-runs/${runId}-designer.json`, `${JSON.stringify(run, null, 2)}\n`);
